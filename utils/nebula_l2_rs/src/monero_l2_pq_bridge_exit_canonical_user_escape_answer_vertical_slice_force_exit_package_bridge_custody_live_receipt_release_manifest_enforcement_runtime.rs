...
pub type MoneroL2PqBridgeExitCanonicalUserEscapeAnswerVerticalSliceForceExitPackageBridgeCustodyLiveReceiptReleaseManifestEnforcementRuntimeResult<
    T,
> = Result<T>;
pub type Runtime = State;
pub const MONERO_L2_PQ_BRIDGE_EXIT_CANONICAL_USER_ESCAPE_ANSWER_VERTICAL_SLICE_FORCE_EXIT_PACKAGE_BRIDGE_CUSTODY_LIVE_RECEIPT_RELEASE_MANIFEST_ENFORCEMENT_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-monero-l2-pq-bridge-exit-canonical-user-escape-answer-vertical-slice-force-exit-package-bridge-custody-live-receipt-release-manifest-enforcement-runtime-v1";
pub const PROTOCOL_VERSION: &str = MONERO_L2_PQ_BRIDGE_EXIT_CANONICAL_USER_ESCAPE_ANSWER_VERTICAL_SLICE_FORCE_EXIT_PACKAGE_BRIDGE_CUSTODY_LIVE_RECEIPT_RELEASE_MANIFEST_ENFORCEMENT_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const RELEASE_MANIFEST_ENFORCEMENT_SUITE: &str =
    "monero-l2-pq-force-exit-package-bridge-custody-live-receipt-release-manifest-enforcement-v1";
...
pub struct Config {
    ...
    pub release_manifest_enforcement_suite: String,
    ...
    pub require_release_manifest_policy_root: bool,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            release_manifest_enforcement_suite: RELEASE_MANIFEST_ENFORCEMENT_SUITE.to_string(),
            ...
            require_release_manifest_policy_root: true,
...
impl Config {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "release_manifest_enforcement_suite": self.release_manifest_enforcement_suite,
            ...
            "require_release_manifest_policy_root": self.require_release_manifest_policy_root,
...
pub enum ManifestEnforcementStatus {
    Activated,
    Held,
    Rejected,
}
impl ManifestEnforcementStatus {
...
pub enum HoldKind {
    ...
    MissingActivationAdjudicator,
    MissingReleaseManifestPolicy,
    MismatchDetected,
}
impl HoldKind {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::MissingActivationAdjudicator => "missing_activation_adjudicator",
            Self::MissingReleaseManifestPolicy => "missing_release_manifest_policy",
...
pub struct Roots {
    ...
    pub release_manifest_policy_root: String,
    pub mismatch_hold_root: String,
    pub release_manifest_verdict_root: String,
...
impl Roots {
    pub fn new(
        ...
        Self {
            ...
            release_manifest_policy_root: String::new(),
            ...
            release_manifest_verdict_root: String::new(),
            fail_closed_root: String::new(),
            release_manifest_enforcement_root: String::new(),
        }
    }
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "release_manifest_policy_root": self.release_manifest_policy_root,
            "mismatch_hold_root": self.mismatch_hold_root,
            "release_manifest_verdict_root": self.release_manifest_verdict_root,
...
impl ReleaseManifestEnforcementVerdict {
    pub fn new(config: &Config, roots: &Roots, counters: &Counters) -> Self {
        ...
        let activation_adjudicator_present = !roots.activation_adjudicator_root.is_empty();
        let release_manifest_policy_present = !roots.release_manifest_policy_root.is_empty();
        ...
        let required_roots_present = !roots.custody_signer_root.is_empty()
            ...
            && !roots.activation_adjudicator_root.is_empty()
            && !roots.release_manifest_policy_root.is_empty()
        ...
        let required_checks_met =
            optional_requirement(config.require_signer_custody_quorum, signer_quorum_met)
                ...
                && optional_requirement(
                    ...
                    activation_adjudicator_present,
                )
                && optional_requirement(
                    config.require_release_manifest_policy_root,
                    release_manifest_policy_present,
                )
        ...
        let manifest_enforcement_status = if release_authorized {
            ManifestEnforcementStatus::Activated
        } else if fail_closed {
            ManifestEnforcementStatus::Rejected
        } else {
            ManifestEnforcementStatus::Held
        };
        ...
        let verdict_root = release_manifest_verdict_root(
            config,
            roots,
            counters,
            fail_closed_verdict,
            manifest_enforcement_status,
            hold_kind,
            release_authorized,
        );
        Self {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "activation_adjudicator_present": self.activation_adjudicator_present,
            "release_manifest_policy_present": self.release_manifest_policy_present,
            "mismatch_holds_clear": self.mismatch_holds_clear,
            "fail_closed_verdict": self.fail_closed_verdict.as_str(),
            "manifest_enforcement_status": self.manifest_enforcement_status.as_str(),
...
pub struct State {
    ...
    pub verdict: ReleaseManifestEnforcementVerdict,
}
impl State {
    pub fn new(
        config: Config,
        ...
        roots.activation_adjudicator_root = activation_adjudicator_root(&config, &roots, &counters);
        roots.release_manifest_policy_root =
            release_manifest_policy_root(&config, &roots, &counters);
        let verdict = ReleaseManifestEnforcementVerdict::new(&config, &roots, &counters);
        roots.release_manifest_verdict_root = record_root(
            "release-manifest-enforcement-verdict",
            &verdict.public_record(),
        );
        roots.fail_closed_root = record_root(
            "fail-closed",
            &json!({
                "fail_closed_verdict": verdict.fail_closed_verdict.as_str(),
                "manifest_enforcement_status": verdict.manifest_enforcement_status.as_str(),
                "hold_kind": verdict.hold_kind.as_str(),
                "release_authorized": verdict.release_authorized,
            }),
        );
        roots.release_manifest_enforcement_root =
            release_manifest_enforcement_root(&config, &roots, &counters, &verdict);
        Ok(Self {
            config,
            custody_signers,
            monero_observations,
    ...
    pub fn state_root(&self) -> String {
        domain_hash(
            "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-STATE",
...
pub fn devnet() -> State {
    ...
    roots.activation_adjudicator_root = activation_adjudicator_root(&config, &roots, &counters);
    roots.release_manifest_policy_root = release_manifest_policy_root(&config, &roots, &counters);
    let verdict = ReleaseManifestEnforcementVerdict::new(&config, &roots, &counters);
    roots.release_manifest_verdict_root = record_root(
        "release-manifest-enforcement-verdict",
        &verdict.public_record(),
...
pub fn record_root(kind: &str, record: &Value) -> String {
    domain_hash(
        "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-RECORD",
...
fn vector_record_root(kind: &str, records: &[Value]) -> String {
    let leaves = records
        ...
        .collect::<Vec<_>>();
    merkle_root(
        "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-VECTOR",
        &leaves,
    )
}
fn release_manifest_verdict_root(
    config: &Config,
    roots: &Roots,
    counters: &Counters,
    fail_closed_verdict: FailClosedVerdict,
    manifest_enforcement_status: ManifestEnforcementStatus,
    hold_kind: HoldKind,
    release_authorized: bool,
) -> String {
    domain_hash(
        "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-VERDICT",
        &[
            HashPart::Str(&config.release_manifest_enforcement_suite),
            ...
            HashPart::Str(&roots.release_manifest_policy_root),
            ...
            HashPart::Str(manifest_enforcement_status.as_str()),
...
fn activation_adjudicator_root(config: &Config, roots: &Roots, counters: &Counters) -> String {
    domain_hash(
        "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-ACTIVATION-ADJUDICATOR",
        &[
            HashPart::Str(&config.release_manifest_enforcement_suite),
            HashPart::Str(&roots.custody_signer_root),
            HashPart::Str(&roots.monero_release_observation_root),
...
fn release_manifest_policy_root(config: &Config, roots: &Roots, counters: &Counters) -> String {
    domain_hash(
        "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-POLICY",
        &[
            HashPart::Str(&config.release_manifest_enforcement_suite),
            HashPart::Str(&roots.activation_adjudicator_root),
            HashPart::Str(&roots.mismatch_hold_root),
            HashPart::U64(config.min_custody_signers),
            HashPart::U64(config.min_monero_observers),
            HashPart::U64(config.min_reserve_handoffs),
            HashPart::U64(config.challenge_window_blocks),
            HashPart::U64(config.required_confirmations),
            HashPart::U64(counters.active_mismatch_hold_count),
        ],
        32,
    )
}
fn release_manifest_enforcement_root(
    config: &Config,
    roots: &Roots,
    counters: &Counters,
    verdict: &ReleaseManifestEnforcementVerdict,
) -> String {
    domain_hash(
        "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-COMMITMENT",
        &[
            ...
            HashPart::Str(verdict.manifest_enforcement_status.as_str()),
...
fn deterministic_id(kind: &str, parts: &[&str]) -> String {
    let leaves = parts
        ...
        .collect::<Vec<_>>();
    domain_hash(
        "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-ID",
        &[
            HashPart::Str(CHAIN_ID),
            HashPart::Str(kind),
            HashPart::Str(&merkle_root(
                "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-ID-PARTS",
                &leaves,
            )),
        ],
        32,
...
fn sample_root(kind: &str, label: &str) -> String {
    domain_hash(
        "MONERO-L2-PQ-BRIDGE-CUSTODY-LIVE-RECEIPT-RELEASE-MANIFEST-ENFORCEMENT-DEVNET-SAMPLE",
        &[
            HashPart::Str(CHAIN_ID),
            HashPart::Str(kind),
            HashPart::Str(label),
...
fn first_hold_kind(
    ...
    activation_adjudicator_present: bool,
    release_manifest_policy_present: bool,
    ...
    } else if !activation_adjudicator_present {
        HoldKind::MissingActivationAdjudicator
    } else if !release_manifest_policy_present {
        HoldKind::MissingReleaseManifestPolicy
...
fn validate_config(config: &Config) -> Result<()> {
    ...
    if valid {
        ...
        Err("bridge custody live receipt release manifest enforcement config invalid".to_string())
    }
}
