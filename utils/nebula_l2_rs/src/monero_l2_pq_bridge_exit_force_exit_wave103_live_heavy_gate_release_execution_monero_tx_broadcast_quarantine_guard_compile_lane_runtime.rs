...
const PROTOCOL_VERSION: &str =
    "wave103-live-heavy-gate-release-execution-monero-tx-broadcast-quarantine-guard-compile-lane-runtime-v1";
const WAVE: u64 = 103;
const RECONCILIATION_WAVE: u64 = 102;
const MIN_QUARANTINE_CONFIRMATIONS: u64 = 720;
const LANE_ID: &str =
    "wave103-live-heavy-gate-release-execution-monero-tx-broadcast-quarantine-guard-compile";
...
pub struct BroadcastPolicy {
    ...
    pub broadcast_plan_policy_root: String,
    pub fee_cap_policy_root: String,
    pub decoy_privacy_policy_root: String,
...
impl BroadcastPolicy {
    pub fn new(lane: LaneKind, claim_label: &str, ordinal: u64) -> Self {
        ...
        let command_hint = format!(
            "nebula wave103 broadcast-quarantine --lane {} --claim {} --hold-broadcast",
            lane.as_str(),
            claim_label
        );
        let quarantine_hold_root =
            label_root("quarantine_hold", lane.as_str(), claim_label, ordinal);
        let broadcast_plan_policy_root =
            label_root("broadcast_plan_policy", lane.as_str(), claim_label, ordinal);
        let fee_cap_policy_root = label_root("fee_cap_policy", lane.as_str(), claim_label, ordinal);
        let decoy_privacy_policy_root =
            label_root("decoy_privacy_policy", lane.as_str(), claim_label, ordinal);
        ...
        Self {
            ...
            broadcast_plan_policy_root,
            fee_cap_policy_root,
            decoy_privacy_policy_root,
    ...
    pub fn public_record(&self) -> PublicRecord {
        json!({
            ...
            "broadcast_plan_policy_root": self.broadcast_plan_policy_root,
            "fee_cap_policy_root": self.fee_cap_policy_root,
            "decoy_privacy_policy_root": self.decoy_privacy_policy_root,
...
impl ReleaseBroadcastQuarantine {
    ...
    pub fn stage_broadcast(
        ...
        self.status = if self.blockers.is_empty() {
            BroadcastQuarantineStatus::Authorized
        } else if self.roots.decoy_privacy_root.is_some() {
            BroadcastQuarantineStatus::PrivacyChecked
    ...
    pub fn release_broadcast(mut self, config: &Config) -> Result<Self> {
        self.blockers = self.active_blockers(config);
...
impl State {
    ...
    pub fn planned_root(&self) -> String {
        status_root(
            "wave103_monero_broadcast_plan_candidates",
            &self.checkpoints,
            BroadcastQuarantineStatus::Planned,
        )
    }
    ...
    pub fn privacy_checked_root(&self) -> String {
        status_root(
            "wave103_monero_broadcast_privacy_checked_candidates",
            &self.checkpoints,
            BroadcastQuarantineStatus::PrivacyChecked,
        )
    }
    ...
    pub fn authorized_root(&self) -> String {
        status_root(
            "wave103_monero_broadcast_authorized_candidates",
            &self.checkpoints,
            BroadcastQuarantineStatus::Authorized,
        )
    }
    ...
    pub fn released_root(&self) -> String {
        status_root(
            "wave103_monero_broadcast_released_claims",
            &self.checkpoints,
    ...
    pub fn broadcast_denial_root(&self) -> String {
        let blocker_labels = self
            ...
            .collect::<Vec<_>>();
        root_from_strings("wave103_monero_broadcast_denial_blockers", blocker_labels)
    }
...
pub fn devnet() -> State {
    ...
    let claim_labels = [
        "wave102_reconciliation_release_hold",
        "monero_release_transaction_plan",
        "monero_fee_cap_envelope",
        "monero_decoy_privacy_envelope",
        "pq_authorized_broadcast_release",
        "mempool_quarantine_observation",
        "operator_reviewer_broadcast_signoff",
    ];
