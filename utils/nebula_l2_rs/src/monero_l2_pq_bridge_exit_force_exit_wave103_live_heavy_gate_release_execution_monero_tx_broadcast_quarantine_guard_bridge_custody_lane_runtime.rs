...
const PROTOCOL_VERSION: &str = "wave103-live-heavy-gate-release-execution-monero-tx-broadcast-quarantine-guard-bridge-custody-lane-runtime-v1";
const WAVE: u64 = 103;
const RECONCILIATION_WAVE: u64 = 102;
const MIN_QUARANTINE_CONFIRMATIONS: u64 = 720;
const MAX_RELEASE_FEE_BPS: u64 = 5;
const LANE_ID: &str =
    "wave103-live-heavy-gate-release-execution-monero-tx-broadcast-quarantine-guard-bridge-custody";
...
impl LaneKind {
    ...
    pub fn title(self) -> &'static str {
        match self {
            Self::BridgeCustody => "Bridge custody Monero transaction broadcast quarantine guard",
        }
    }

    pub fn command_scope(self) -> &'static str {
        match self {
            Self::BridgeCustody => "bridge-custody-broadcast-quarantine",
        }
    }
}
...
impl BroadcastStatus {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CustodyChecked => "custody_checked",
...
impl BroadcastBlockerKind {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::MissingCustodySpendRoot => "missing_custody_spend_root",
            ...
            Self::ReleaseFeeBpsTooHigh => "release_fee_bps_too_high",
...
pub struct Config {
    ...
    pub require_custody_spend_root: bool,
...
impl Config {
    pub fn public_record(&self) -> PublicRecord {
        json!({
            ...
            "require_custody_spend_root": self.require_custody_spend_root,
...
pub struct BroadcastQuarantineRoots {
    ...
    pub custody_spend_root: Option<String>,
...
impl BroadcastQuarantineRoots {
    pub fn public_record(&self) -> PublicRecord {
        json!({
            ...
            "custody_spend_root": self.custody_spend_root,
...
impl BroadcastMeasurements {
    pub fn blocked(config: &Config) -> Self {
        Self {
            ...
            release_fee_bps: config.max_release_fee_bps.saturating_add(1),
        }
    }
    ...
    pub fn public_record(self) -> PublicRecord {
        json!({
            ...
            "release_fee_bps": self.release_fee_bps,
        })
    }
...
impl BroadcastPolicy {
    pub fn new(lane: LaneKind, claim_label: &str, ordinal: u64) -> Self {
        ...
        let custody_policy_root = label_root("custody_policy", lane.as_str(), claim_label, ordinal);
        ...
        let circuit_breaker_policy_root = label_root(
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct ReleaseBroadcastQuarantine {
    pub lane: LaneKind,
    pub claim_label: String,
    pub ordinal: u64,
...
impl ReleaseBroadcastQuarantine {
    ...
    pub fn stage_quarantine(
        ...
        self.status = if self.blockers.is_empty() {
            BroadcastStatus::Authorized
        } else if self.roots.pq_authorization_root.is_some() {
            BroadcastStatus::PrivacyChecked
        } else if self.roots.decoy_privacy_root.is_some() {
            BroadcastStatus::CustodyChecked
    ...
    pub fn active_blockers(&self, config: &Config) -> Vec<BroadcastBlockerKind> {
        ...
        if config.require_custody_spend_root && self.roots.custody_spend_root.is_none() {
            blockers.push(BroadcastBlockerKind::MissingCustodySpendRoot);
        }
        ...
        if self.measurements.release_fee_bps > config.max_release_fee_bps {
            blockers.push(BroadcastBlockerKind::ReleaseFeeBpsTooHigh);
        }
    ...
    pub fn state_root(&self) -> String {
        record_root("release_broadcast_quarantine", &self.public_record())
    }
}
...
pub struct State {
    ...
    pub checkpoints: Vec<ReleaseBroadcastQuarantine>,
...
impl State {
    pub fn new(
        config: Config,
        lane: LaneKind,
        checkpoints: Vec<ReleaseBroadcastQuarantine>,
    ) -> Self {
    ...
    pub fn planned_root(&self) -> String {
        status_root(
            "wave103_release_transaction_plan_candidates",
            &self.checkpoints,
            BroadcastStatus::Planned,
        )
    }

    pub fn custody_spend_root(&self) -> String {
        status_root(
            "wave103_custody_spend_candidates",
            &self.checkpoints,
            BroadcastStatus::CustodyChecked,
        )
    }
    ...
    pub fn privacy_root(&self) -> String {
        status_root(
            "wave103_decoy_privacy_candidates",
            &self.checkpoints,
            BroadcastStatus::PrivacyChecked,
        )
    }

    pub fn authorized_root(&self) -> String {
        status_root(
            "wave103_pq_authorized_broadcast_candidates",
            &self.checkpoints,
            BroadcastStatus::Authorized,
        )
    }
    ...
    pub fn broadcast_ready_root(&self) -> String {
        status_root(
            "wave103_broadcast_ready_release_transactions",
            &self.checkpoints,
...
pub fn devnet() -> State {
    ...
    let claim_labels = [
        "wave102_reconciliation_gate",
        "release_transaction_plan",
        "custody_spend_authority",
        "fee_cap_enforcement",
        "decoy_privacy_envelope",
        "pq_authorized_release",
    ...
    let checkpoints = claim_labels
        ...
        .map(|(index, claim_label)| {
            ReleaseBroadcastQuarantine::empty(lane, claim_label, (index + 1) as u64, &config)
        })
        .collect::<Vec<_>>();
...
fn initial_blockers(config: &Config) -> Vec<BroadcastBlockerKind> {
    ...
    if config.require_custody_spend_root {
        blockers.push(BroadcastBlockerKind::MissingCustodySpendRoot);
    }
...
fn blocked_root(checkpoints: &[ReleaseBroadcastQuarantine]) -> String {
...
fn status_root(
    domain: &str,
    checkpoints: &[ReleaseBroadcastQuarantine],
