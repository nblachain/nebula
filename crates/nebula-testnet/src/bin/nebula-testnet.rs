...
fn run_rpc_node(args: &[String]) {
    ...
    eprintln!(
        "Nebula RPC listening on {bind_addr}; block target {} ms; validator {}; role {}; sequencer key {}; max mempool {} txs; max request {} bytes; rate limit {} requests/min",
...
RPC OPERATOR OPS AND BACKUP:
    ...
    request-size/rate-limit policy, bridge policy root, and backup manifest
    root before opening a public testnet endpoint.
